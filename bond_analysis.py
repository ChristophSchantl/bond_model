...
code = dedent('''
    ...
    # - Z-Spread Solver auf Zero-Kurve (Upload CSV)
    # - Hazard-Rate / Survival (implizit aus Preis & Recovery)
    # - Key Rate Duration (lokaler Kurven-Bump)
    # - Carry & Roll-down (6M/12M) + Pull-to-Par-Pfad
    # - Stressdeck / Tornado (Yield ±100bp, Spread ±100bp, Recovery, PD)
    # - Zusätzliche Grafiken: Cashflow-Timeline, PV-Profile, Duration-Beiträge,
    #   Price↔Yield, Zero vs Zero+Z, Survival-Kurve, Pull-to-Par, Tornado
    #
    # CSV Zero-Kurve-Format (flexibel):
    #   - Spalten: either ["t_years","zero_rate"]  oder ["tenor","rate"]
    #   - t_years/tenor in Jahren (float), rate als Dezimal (z.B. 0.025) oder % (2.5)
    #
    # Start: streamlit run bond_lab_pro.py
    ...
    def parse_zero_curve(df: pd.DataFrame):
        ...
        df2=df.copy(); df2.columns=cols
        if "t_years" in cols: t=df2["t_years"].astype(float).values
        elif "tenor" in cols: t=df2["tenor"].astype(float).values
        else: raise ValueError("Zero curve CSV benötigt Spalte 't_years' oder 'tenor' (Jahre).")
        if "zero_rate" in cols: r=df2["zero_rate"].astype(float).values
        elif "rate" in cols:    r=df2["rate"].astype(float).values
        else: raise ValueError("Zero curve CSV benötigt Spalte 'zero_rate' oder 'rate'.")
        if np.nanmean(r)>1.0: r=r/100.0
        t=np.array(t,dtype=float); r=np.array(r,dtype=float)
        idx=np.argsort(t); return t[idx], r[idx]
    ...
    def interp_rate(t_nodes, r_nodes, t):
        ...
        t0,t1=t_nodes[i], t_nodes[i+1]; r0,r1=r_nodes[i], r_nodes[i+1]
        w=(t-t0)/(t1-t0+1e-12); return float(r0+w*(r1-r0))
    ...
    def price_with_hazard_FRP(cashflows, settlement, t_nodes, r_nodes, lam, R, nominal):
        ...
        for dt, cf in cashflows:
            t=(dt-settlement).days/365.0
            S=math.exp(-lam*t); df=df_from_zero_curve(t_nodes, r_nodes, t)
            pv += cf*df*S
    ...
    with st.sidebar:
        ...
        zero_file = st.file_uploader("Zero Curve CSV (t_years, zero_rate)", type=["csv"])
        t_nodes, r_nodes = None, None
        if zero_file is not None:
            try:
                zdf=pd.read_csv(zero_file); t_nodes, r_nodes = parse_zero_curve(zdf)
    ...
    with tab_pricing:
        ...
        cf_df["pv_weight"]=cf_df["pv"]/cf_df["pv"].sum()

        fig_cf=go.Figure(); fig_cf.add_bar(x=cf_df["date"], y=cf_df["cf"], name="Cashflows")
        fig_cf.update_layout(height=300, template="plotly_white", yaxis_title="Betrag")
        st.plotly_chart(fig_cf, use_container_width=True)

        fig_pv=go.Figure(); fig_pv.add_bar(x=cf_df["date"], y=cf_df["pv"], name="PV")
    ...
    with tab_credit:
        if t_nodes is None:
            t_nodes = np.array([0.0, 30.0]); r_nodes=np.array([bench_flat/100.0, bench_flat/100.0])
        ...
        Rs=np.linspace(0.0, 0.6, 13); lam_R=[solve_hazard_from_price(dirty_abs, cashflows, settlement, t_nodes, r_nodes, R, nominal) for R in Rs]
    ...
    with tab_stress:
        ...
        lam0=solve_hazard_from_price(model_dirty, cashflows, settlement, t_nodes if t_nodes is not None else np.array([0.0,30.0]), r_nodes if r_nodes is not None else np.array([bench_flat/100.0, bench_flat/100.0]), recov, nominal)
        for R in [0.10,0.40]:
            pR=price_with_hazard_FRP(cashflows, settlement, t_nodes if t_nodes is not None else np.array([0.0,30.0]), r_nodes if r_nodes is not None else np.array([bench_flat/100.0, bench_flat/100.0]), lam0, R, nominal)
            scenarios.append((f"Recovery {int(R*100)}%", (pR-model_dirty)/nominal*100.0))
        for s in [0.5,1.5]:
            lam_s=lam0*s
            ps=price_with_hazard_FRP(cashflows, settlement, t_nodes if t_nodes is not None else np.array([0.0,30.0]), r_nodes if r_nodes is not None else np.array([bench_flat/100.0, bench_flat/100.0]), lam_s, recov, nominal)
...
with open('/mnt/data/bond_lab_pro.py', 'w', encoding='utf-8') as f:
    f.write(code)

print("/mnt/data/bond_lab_pro.py created")
